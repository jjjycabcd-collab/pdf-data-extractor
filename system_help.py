...
<style>
    ...
    html, body, [data-testid="stAppViewContainer"] {
        font-family: 'Pretendard', sans-serif; background-color: #fafaf9; color: #292524;
    }
    
    /* 탭(Tab) 커스텀 UI 스타일 */
    div[data-baseweb="tab-list"] { gap: 6px; border-bottom: 2px solid #e7e5e4; }
    button[data-baseweb="tab"] { background-color: #f5f5f4 !important; border: 1px solid #e7e5e4 !important; border-bottom: none !important; border-radius: 8px 8px 0 0 !important; padding: 10px 20px !important; color: #57534e !important; }
    button[data-baseweb="tab"][aria-selected="true"] { background-color: #ffffff !important; border-top: 3px solid #0d9488 !important; border-left: 1px solid #e7e5e4 !important; border-right: 1px solid #e7e5e4 !important; color: #0d9488 !important; font-weight: 700 !important; margin-bottom: -2px !important; }
    div[data-testid="stTabView"] > div { padding-top: 1.5rem; }

    .main-title { font-size: 2rem; font-weight: 800; color: #1c1917; text-align: center; margin-bottom: 0.5rem; }
    .sub-title { font-size: 1.1rem; color: #292524; text-align: center; margin-bottom: 2rem; max-width: 800px; margin-left: auto; margin-right: auto; line-height: 1.6; }
    .step-box { background-color: #f5f5f4; padding: 1.5rem; border-radius: 0.75rem; border: 1px solid #e7e5e4; min-height: 200px; margin-bottom: 1.5rem; }
    .limit-card { background-color: #f5f5f4; padding: 1.5rem; border-radius: 0.75rem; border: 1px solid #e7e5e4; height: 100%; }
    .accent-text { color: #0d9488; font-weight: 600; }
...
with tab1:
    ...
    with step_cols[0]:
        selected_step = st.radio("파이프라인 단계 선택", options=list(workflow_data.keys()), label_visibility="collapsed")
...
with tab2:
    ...
    <div class="arch-layer">
        <div class="arch-title">🌐 Client Layer (Presentation)</div>
        <div class="arch-grid">
            <div><div class="arch-box"><h5>🖥️ Web Browser UI</h5><p>- Streamlit & Drawable Canvas<br>- 사용자 Bbox 드래그 및 파라미터 제어</p></div></div>
            <div><div class="arch-box"><h5>⚡ DOM Injector</h5><p>- Custom JS 동적 주입<br>- 단축키 이벤트 & 교정 커서 동기화</p></div></div>
        </div>
    </div>
    <div class="arrow-down">⬇️ WebSockets / HTTP Request</div>
    <div class="arch-layer">
        <div class="arch-title">🧠 Core Application Layer (Business Logic)</div>
        <div class="arch-grid">
            <div><div class="arch-box"><h5>📄 Native Extractor</h5><p>- PyMuPDF (fitz) 엔진<br>- 텍스트 좌표 맵핑 & 오토피팅 제어</p></div></div>
            <div><div class="arch-box"><h5>👁️ OCR Processing</h5><p>- PyTesseract / PIL 모듈<br>- 다국어 인식 & 이미지 전처리</p></div></div>
            <div><div class="arch-box"><h5>🔄 State & Data Controller</h5><p>- Streamlit Session State 관리<br>- 데이터 병합 및 그룹핑 연산</p></div></div>
        </div>
    </div>
    <div class="arrow-down">⬇️ Data Parsing & Serialization</div>
    <div class="arch-layer">
        <div class="arch-title">💾 Infrastructure & Storage Layer</div>
        <div class="arch-grid">
            <div><div class="arch-box"><h5>☁️ Cloud Environment</h5><p>- Python 3.11 Runtime<br>- Host OS Binary (Tesseract-OCR)</p></div></div>
            <div><div class="arch-box"><h5>📦 Output Artifacts</h5><p>- 구축 메타데이터 JSON Schema<br>- Markdown 구조화 문서 생성</p></div></div>
        </div>
    </div>
    ''', unsafe_allow_html=True)
...
with tab3:
    ...
    with tech_cols[1]:
        ...
        fig.add_trace(go.Scatterpolar(
            r=tech_data[selected_tech]["metrics"], theta=categories, fill='toself',
            fillcolor='rgba(13, 148, 136, 0.2)', line=dict(color='rgba(13, 148, 136, 1)', width=2), marker=dict(color='rgba(13, 148, 136, 1)')
        ))
        fig.update_layout(
            polar=dict(radialaxis=dict(visible=False, range=[0, 100]), angularaxis=dict(tickfont=dict(family="Pretendard", size=12))),
            showlegend=False, margin=dict(l=40, r=40, t=20, b=20), height=300, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'
        )
...
with tab5:
        ...
        "1. 아키텍처 및 라이브러리 맵핑 (Analysis)": {
            ...
            "content": """
                - **Python -> Java 대체 모듈 선정:**
                  - `PyMuPDF` ➔ 오픈소스(`Apache PDFBox`, `iText`) 또는 상용 솔루션(`Aspose.PDF for Java`) 적용
    ...
    with egov_cols[0]:
        selected_egov_step = st.radio("마이그레이션 단계 선택", options=list(egov_steps.keys()), key="egov_radio", label_visibility="collapsed")
    with egov_cols[1]:
    ...
    <table class="compare-table">
        <thead>
            <tr><th width="15%">구분</th><th width="35%">AS-IS (현재 Python 환경)</th><th width="35%">TO-BE (eGov 3.9 환경)</th><th width="15%">기대 효과</th></tr>
        </thead>
        <tbody>
            <tr><td>프론트엔드 UI</td><td>Streamlit (Python 기반 동적 렌더링)</td><td>JSP, HTML5 Canvas (Fabric.js), AJAX</td><td>표준 웹 접근성 및 렌더링 최적화</td></tr>
            <tr><td>백엔드 프레임워크</td><td>Streamlit 단일 스레드 (Session State)</td><td>eGovFrame 3.9 (Spring MVC, Java)</td><td>대용량 트래픽 처리 및 안정성</td></tr>
            <tr><td>PDF 파싱 및 추출</td><td>PyMuPDF (fitz)</td><td>Apache PDFBox / iText <br><span class="highlight-text">+ Aspose.PDF (고성능/유료)</span></td><td>엔터프라이즈급 정밀 렌더링 및 좌표 매핑</td></tr>
            <tr><td>이미지 OCR 엔진</td><td>PyTesseract (Python)</td><td>Tess4J (Tesseract JNA 래퍼)</td><td>멀티스레딩 병렬 처리 지원</td></tr>
            <tr><td>텍스트 교정(Diff)</td><td>difflib (Python)</td><td>java-diff-utils (Java)</td><td>동일 수준의 시각적 알고리즘 유지</td></tr>
            <tr><td>데이터 및 상태 관리</td><td>Session State (브라우저 종속 인메모리)</td><td><span class="highlight-text">Oracle</span> + MyBatis</td><td>데이터 영구 저장 및 무결성 보장</td></tr>
        </tbody>
    </table>
    ''', unsafe_allow_html=True)
...
with tab6:
    ...
    st.markdown('''
    <div class="arch-layer">
        <div class="arch-title">🌐 Presentation Layer (Client & Web)</div>
        <div class="arch-grid">
            <div><div class="arch-box"><h5>🖥️ JSP & HTML5 UI</h5><p>- eGov UI 표준 템플릿<br>- Bootstrap / Tailwind CSS</p></div></div>
            <div><div class="arch-box"><h5>🎨 Fabric.js (Canvas)</h5><p>- PDF 렌더링 및 오버레이<br>- Bbox 드로잉 & 이벤트 리스너</p></div></div>
            <div><div class="arch-box"><h5>⚡ REST / Ajax</h5><p>- JSON 기반 비동기 데이터 통신<br>- 좌표 및 텍스트 교환</p></div></div>
        </div>
    </div>
    <div class="arrow-down">⬇️ Spring DispatcherServlet</div>
    <div class="arch-layer">
        <div class="arch-title">🧠 Business Logic Layer (eGovFrame 3.9)</div>
        <div class="arch-grid">
            <div><div class="arch-box"><h5>🎯 Spring Controller</h5><p>- Request 매핑 및 권한 검증<br>- View(JSP) 및 API 라우팅</p></div></div>
            <div><div class="arch-box"><h5>📄 Java PDF 엔진</h5><p>- PDFBox 또는 Aspose.PDF<br>- PDF Document 파싱 및 텍스트 추출</p></div></div>
            <div><div class="arch-box"><h5>👁️ Tess4J (OCR)</h5><p>- Tesseract 엔진 Java 연동<br>- 이미지 크롭 및 문자인식<br>- java-diff-utils 교정 로직</p></div></div>
        </div>
    </div>
    <div class="arrow-down">⬇️ MyBatis ORM</div>
    <div class="arch-layer">
        <div class="arch-title">💾 Data Access Layer (DBMS & Storage)</div>
        <div class="arch-grid">
            <div><div class="arch-box"><h5>🗄️ Relational DB (RDBMS)</h5><p>- Oracle<br>- 문서 메타, Bbox 좌표, 텍스트 데이터 영구 저장</p></div></div>
            <div><div class="arch-box"><h5>📂 File System / NAS</h5><p>- 원본 PDF 파일 저장소<br>- 추출 완료된 학습용 JSON/XML 산출물 관리</p></div></div>
        </div>
    </div>
    ''', unsafe_allow_html=True)
